...
class ContextualModel(object):
    ...
    def create_start_parameter(self, dataset):
        ...
        if self.name == 'linear':
            ...
            return np.concatenate([np.ones((1,d[1])), self.rng.normal(scale=dataset.parameter_scale, size=d)],axis=0)
    ...
    def _linear_modelling(self, parameter, features):
        ...
        if isinstance(features, list):
            ...
            for j in range(len(features)):
                context_theta.append(np.dot(features[j], mean_coeff) + intercept_coeff)
    ...
    def _polynomial_modelling(self, parameter, features):
        ...
        if isinstance(features, list):
            context_theta = []
            for j in range(len(features)):
                f = np.einsum("ij,ih->ijh", features[j], features[j]).reshape(
                    features[j].shape[0], -1
                )
                m_kern = np.dot(f, coeff_kern)
                m_linear = np.dot(features[j], coeff_lin) + intercept
                mean = m_kern + m_linear
                context_theta.append(mean)
            return context_theta
        
        else:
            m_linear = np.dot(features, coeff_lin) + intercept
            f = np.einsum("ij,ih->ijh", features, features).reshape(
                features.shape[0], -1
            )
            m_kern = np.dot(f, coeff_kern)
            mean = m_kern + m_linear
    ...
    def get_parameter(self, parameter, features):
        if self.name == "linear":
            ...
            if parameter.shape[1]>1:
                ...
                return self._linear_modelling(parameter, features).reshape(-1,)
